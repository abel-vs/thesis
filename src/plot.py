...
def print_header(title = ""):
    if (title == ""):
        ...
        print("="*half_header + " " + title.upper() + " " + "="*half_header)
...
def print_metrics(**kwargs):
    ...
    if 'batch_duration' in kwargs and 'batch_size' in kwargs: 
        print("Time per batch: {:.4f} ms ({} per batch)".format(kwargs.get("batch_duration"), kwargs.get("batch_size")))
    ...
    if 'model' in kwargs:
        ...
        if 'example_input' in kwargs:
            model_flops, params = eval.get_model_flops(kwargs.get("model"), kwargs.get("example_input").size())
...
def print_before_after_metrics(before, after):
    ...
    if in_both('score'): 
        print("Score: {:.6f} -> {:.6f} ".format(before.get("score"), after.get("score")))
    if in_both('batch_duration') and in_both("batch_size"): 
        assert before.get("batch_size") == after.get("batch_size")
        print("Time per batch: {:.4f} ms -> {:.4f} ms ({} per batch)".format(before.get("batch_duration"), after.get("batch_duration"), before.get("batch_size")))
    if in_both('data_duration'): 
        print("Time per data point: {:.4f} ms -> {:.4f} ms".format(before.get("data_duration"), after.get("data_duration")))
    if in_both('model'):
        ...
        if in_both('example_input'):
            before_flops, _ = eval.get_model_flops(before.get("model"), before.get("example_input").size())
            after_flops, _ = eval.get_model_flops(after.get("model"), after.get("example_input").size())
