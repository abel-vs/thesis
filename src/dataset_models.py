...
supported_datasets.update({
    ...
    "CIFAR-10": DataSet(
        ...
        criterion=F.cross_entropy,
        metric=metrics.accuracy,
        train_loader=DataLoader(CIFAR10('../data', train=True, download=True, transform=train_cifar_transform),
                                batch_size=64, shuffle=True, **kwargs),
        test_loader=DataLoader(CIFAR10('../data', train=False, download=True, transform=test_cifar_transform),
                               batch_size=64, shuffle=True, **kwargs),
    ),
})
